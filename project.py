...
def detect(img, img0, video_name, username, token):
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if results.multi_face_landmarks:
            ...
            Leftear = (Letr_Y - Lebl_Y + Letl_Y - Lebr_Y) / (2.0 * (Leo_X - Lei_X) + 1e-5)
            ...
            Rightear = (Retr_Y - Rebl_Y + Retl_Y - Rebr_Y) / (2.0 * (Rei_X - Reo_X) + 1e-5)
        ...
        tmp_data = base64.b64encode(tmp)

        tmp_id = str(uuid.uuid4())

        # 如果有异常状态
        if yawning or calling or eye_closed or smoking:
...
def socket(ws, token, username):
    while not ws.closed:
        ...
        if msg != "ping" and msg is not None:
            ...
            vid_reader = imageio.get_reader(imageData, 'ffmpeg')
